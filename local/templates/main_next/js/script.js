$(document).ready(function() {
    if( $('.articles').length ) {
        ...
        if(ARTICLES_PAGES_COUNT > 1) {
            ...
            if( ARTICLES_PAGES_COUNT !== ARTICLES_PAGES[ARTICLES_PAGES.length - 1].data('page')) {
                var isCanLoad = true;
            }

            function loadArticles(scrollOffsetTop) {
                var CURRENT_ARTICLE_PAGE = ARTICLES_PAGES[ARTICLES_PAGES.length - 1],
                    offsetToBottomLastPage = CURRENT_ARTICLE_PAGE.offset().top + CURRENT_ARTICLE_PAGE.outerHeight();

                /*Условия срабатывания загрузки следующей страницы*/
                if (scrollOffsetTop + screenHeight > offsetToBottomLastPage && isCanLoad) {
                    isCanLoad = false;
                    ...
                    $.get(url, function(data) {
                        ...
                        window.history.replaceState({}, '', ARTICLES_URL + 'page_' + (curCountPage + 1));
        ...
        function initMoveToolbars() {
            ...
            return function move(scrollOffsetTop) {
                ...
                if(screenOffsetTop >= rightOffsetTop) {
                    ...
                    right.css({'top': screenOffsetTop - right.offset().top + addOffset + 'px'});
                }else {
                    right.removeAttr('style');
                }
            }
...
function changeBasket(elem) {
    if($('.basket').length) {
        ...
        if( firstParent.data('fieldwidth') ) {
            $.post('/basket/changeBasket.php',{id: id, groupType: groupType, count: count, width: firstParent.data('fieldwidth')}, function() {

            }).fail(function() { alert("error"); });
        }else {
            $.post('/basket/changeBasket.php',{id: id, groupType: groupType, count: count, measure: measure}, function() {

            }).fail(function() { alert("error"); });

            var basketInfo = $('.basketHeaderInfo');

            $.post('/basket/', function(data) { //меняем итоги тотла в шапке корзины
...
$(document).ready(function() {
    if($('.basket').length) {
        ...
        $(document).on('click', '.basket__removeItem', function() {
            ...
            if(id) {
                $.post(url,{id: id, groupType: groupType, measure: measure}, function(data) {
                    parent.empty().remove();
...
$(document).ready(function() {
    function btnEnterHover(elem, event) {
        ...
        elem.find('div:first-child').css({top:relY, left:relX, 'width': elem.outerWidth() * 2 +'px', 'height': elem.outerWidth() * 2 +'px'});
    }
    function btnLeaveHover(elem, event) {
        ...
        elem.find('div:first-child').css({top:relY, left:relX, 'width': 0, 'height': 0});
    }
...
function ajaxReloadContetn(url) { // ф-ия вызывается в компаненте catalog.smart.filter шаблоне filter_to_cataloga
        ...
        $(document)
            .on('mouseenter', '.charts__prop', function() {
                ...
                $.each(charts, function(i, elem) {
                    var curChartElem = $(elem).children('li').eq(curIndex + 1),
...
function addErrorMessageToForm(parentElement, fieldToForm, text) {
    if(!fieldToForm.classList.contains('feedbackFields_error') && !fieldToForm.classList.contains('feedback-form__field_warning')) {
    ...
    for(var i = 0; i < parentElement.children.length; i++) {
        if(parentElement.children[i].classList.contains('feedbackFields__warning-message')) {
...
$(document).ready(function() {
    ...
    $(document).on("blur","input[type='tel']",function(){
        ...
        if(this.value) {
            var inputPhoneValid = /\+7\s\(\d{3}\)\s\d{3}-\d{2}-\d{2}/.test(this.value);
            if(inputPhoneValid) {
                ...
                addErrorMessageToForm(parentElement, parentElement,'Это поле заполнено неверно');
            }
        }else {
            addErrorMessageToForm(parentElement, parentElement,'Это поле нужно заполнить');
        }
    });
    ...
    $(document).on("blur","input[type='email']",function(){
        ...
        if(this.value) {
            var inputMailValid = /^[-a-z0-9!#$%&'*+/=?^_`{|}~]+(?:\.[-a-z0-9!#$%&'*+/=?^_`{|}~]+)*@(?:[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])?\.)*(?:aero|arpa|asia|biz|cat|com|coop|edu|gov|info|int|jobs|mil|mobi|museum|name|net|org|pro|rocks|tel|travel|[a-z][a-z])$/.test(this.value);
...
$(document).ready(function() {
    ...
    $(document).on("click", ".feedbackForm__close", function () {
        feedbackForm.stop().fadeOut(500, function() {
...
function initScriptToSelectOrderBtn() { //ф-ия вызывается в компоненте productSelection
    $(document)
                    ...
                    $.post(url, {
                        ...
                        function(data) {
                            if($('.basketSmallBtn__counter').length) {
                                $('.basketSmallBtn__counter').html($(data).find('.basketSmallBtn__text'));
                            }else {
                                $('.basketSmallBtn').append($(data).find('.basketSmallBtn__counter'));
                            }
    ...
    $('.button_style2')
        .on('mouseover', function(event) {
            ...
            $(this).find('div:first-child').css({top:relY, left:relX, 'width': $(this).outerWidth() * 2 +'px', 'height': $(this).outerWidth() * 2 +'px'});
        })
        .on('mouseout', function(event) {
            ...
            $(this).find('div:first-child').css({top:relY, left:relX, 'width': 0, 'height': 0});
        });
...
$(document).ready(function() {
    if($('.firstScreen').length) {
        var firstScreenImg = $('.firstScreen__img'),
            firstScreenImgUrl = firstScreenImg.css('background-image').slice(4, -1).replace(/"/g, "");
        ...
        function firstScreenAnimation() {
            ...
            return function move(scrollOffsetTop) {
                img.css({'transform': 'translate3D(0,' + scrollOffsetTop * 0.2 + 'px, 0)'});
                title.css({'transform': 'translate3D(0,' + scrollOffsetTop * 0.3 + 'px, 0)'});
                mainBg.css({'transform': 'translate3D(0,' + scrollOffsetTop * 0.4 + 'px, 0)'});
                gradient.css({'transform': 'translate3D(0,' + scrollOffsetTop * -0.2 + 'px, 0)'});
            }
        }
...
$(document).ready(function() {
    ...
    $(document).on("change", ".counter__field", function () {
        ...
        if(value <= 0){
            $(this).parents(".listDetail__row").find(".listDetail__button").addClass("disabled");
        }else{
            $(this).parents(".listDetail__row").find(".listDetail__button").removeClass("disabled");
        }
    });
    ...
    if($('.listDetail').length) {
        ...
        $(document).on('click', '.listDetail__title', function() {
            ...
            var innerItem = $(this).find('span'),
                ...
                isArrowActive = $('.listDetail__title.active').find('.listDetail__icon').hasClass('active');

            if(isActive) { //если активный значит нажали повторно и нужно сменить сортировку
                sortOrder = sortOrder === 'desc' ? 'asc' : 'desc';
            }

            history.replaceState('', '', url); //меняем url и запись в истории

            $.post(url, {sortProperty: sortProperty, sortOrder: sortOrder}, function(data) {
                ...
                var arrow = $('.listDetail__title').eq(index).find('.listDetail__icon');
        ...
        $(document).on('click', '.listDetail__button', function() {
            var patent = $(this).closest('.listDetail__row'),
                ...
                count = $(this).closest('.listDetail__row').find('.counter__field').val();

            if(patent.find('.counter__dimension').length) { //проверка на наличие у counter переключателя измерений
                var measure = patent.find('.counter__btn:checked + label').data('measure');
            }
        ...
        if(NEWS_PAGES_COUNT > 1) {
            ...
            function loadNews(scrollOffsetTop) {
                var CURRENT_NEWS_PAGE = NEWS_PAGES[NEWS_PAGES.length - 1],
                    offsetToBottomLastPage = CURRENT_NEWS_PAGE.offset().top + CURRENT_NEWS_PAGE.outerHeight();
                ...
                if (scrollOffsetTop + screenHeight > offsetToBottomLastPage && isCanLoad) {
                    ...
                    $.get(url, {section: NEWS_SECTION}, function(data) {
                        ...
                        window.history.replaceState({}, '', NEWS_URL + 'page_' + (curCountPage + 1));
        ...
        function initMoveToolbars() {
            ...
            return function move(scrollOffsetTop) {
                ...
                if(screenOffsetTop >= leftOffsetTop) {
                    left.css({'position': 'fixed', 'top':  headerOuterHeight + addOffset + 'px'});
                    right.css({'position': 'fixed', 'top':  headerOuterHeight + addOffset + 'px'});
                }else {
    ...
    if($('.orderFields__submit').length) {
        ...
        $(document).on('click', '.orderFields__submit', function(event) {
            ...
            var btn = $(this).find('.button'),
                isNotError = !$('.feedbackFormBasket').find('.feedbackFields_error').length;

            if(isNotError) {
                btn.addClass('jQ_loader jQ_loader_white').append('<div class="button__loader"><span class="button__loaderOuter"><span></span><span></span><span></span></span></div>');
            }
...
$(window).on("load", function () {
    if($('.pageNavigation').length) {
        ...
        if(/truby/.test(window.location.pathname)) { // если переход по ссылке на трубы то скролим до них
        ...
        function setActiveMenuElemToScrollDown(scrollOffset) {
            var totlaOffset = scrollOffset + headerHeight + correctionOffset + addOffsetTop,
        ...
        function scrollDown(scrollOffset) {
            ...
            if( totalOffset > blockOffsetTop ) {
                blockToScroll.css({'position': 'fixed', top: headerHeight + correctionOffset + 'px'});
            }
            if( (blockToScroll.offset().top + blockToScrollHeight) > (catalogMainOffsetTop + catalogMainHeight) ) {
                blockToScroll.css({'position': 'absolute', top: $('.catalog__sidebar').outerHeight() - blockToScrollHeight - parseInt($('.catalog__content').css('padding-bottom')) + 'px'});
            }
        ...
        $(document).on('scroll', function() {
            ...
            if(curScrollOffset > scrollPrevOffset) { //scroll down
        ...
        $(window).on('resize', function() {
            blockOffsetTop = $('.catalog__sidebar').offset().top + parseInt($('.catalog__sidebar').css('padding-top'));
        });
    }
});
$(document).ready(function() {
    if($('.productSelection').length) {
        function animateFeedbackOrder() {
            var feedbackOrder = $('.feedbackOrder'),
                ...
                paddingProductSelectionBlock = parseInt(productSelection.css("padding-bottom")),
                heightProductSelectionBlock = productSelection.outerHeight(),
                addPadding = heightFeedbackOrderBlock - heightProductSelectionBlock;

            productSelection.css({'opacity': '0', 'padding-bottom': (paddingProductSelectionBlock + addPadding) + 'px'});
    ...
    if( $('.scheme').length ) {
        $(document).on('click', '.scheme__tab', function() {
            ...
            $('.scheme__item.active').fadeOut(250, function() {
                ...
                $('.scheme__item').eq(_$that.index()).fadeIn(250, function() {
                    $(this).addClass('active');
                });
            });
        });
...
function checkCanMainPageScreensAnimation(elem, scrollOffsetTop, removeClass) {
    ...
    if(screenHeight < 921) {
        if(scrollOffsetTop + screenHeight > elemOffsetTop + relativePieceHeightElem) {
...
$(document).ready(function() {
    $(window).scroll(function() {
        if($('.screen2_wrapper.screen2_animation').length) {
            checkCanMainPageScreensAnimation($('.screen2_wrapper.screen2_animation'), $(window).scrollTop(), 'screen2_animation');
        }
        if($('.title__accentText.screen2_animation').length) {
            checkCanMainPageScreensAnimation($('.title__accentText.screen2_animation'), $(window).scrollTop(), 'screen2_animation');
        }
        if($('.title__text.screen2_animation').length) {
            checkCanMainPageScreensAnimation($('.title__text.screen2_animation'), $(window).scrollTop(), 'screen2_animation');
        }
        if($('.screen2__description.screen2_animation').length) {
            checkCanMainPageScreensAnimation($('.screen2__description.screen2_animation'), $(window).scrollTop(), 'screen2_animation');
        }
        if($('.screen2__features.screen2_animation').length) {
            checkCanMainPageScreensAnimation($('.screen2__features.screen2_animation'), $(window).scrollTop(), 'screen2_animation');
        }
        if($('.screen2__btn.screen2_animation').length) {
            checkCanMainPageScreensAnimation($('.screen2__btn.screen2_animation'), $(window).scrollTop(), 'screen2_animation');
        }
    });
});
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        if($('.screen3_wrapper.screen3_animation').length) {
            checkCanMainPageScreensAnimation($('.screen3_wrapper.screen3_animation'), $(window).scrollTop(), 'screen3_animation');
        }
        if($('.clone1.screen3_animation').length) {
            checkCanMainPageScreensAnimation($('.clone1.screen3_animation'), $(window).scrollTop(), 'screen3_animation');
        }
        if($('.clone2.screen3_animation').length) {
            checkCanMainPageScreensAnimation($('.clone2.screen3_animation'), $(window).scrollTop(), 'screen3_animation');
        }
        if($('.screen3__description.screen3_animation').length) {
            checkCanMainPageScreensAnimation($('.screen3__description.screen3_animation'), $(window).scrollTop(), 'screen3_animation');
        }
        if($('.screen3__group1.screen3_animation').length) {
            checkCanMainPageScreensAnimation($('.screen3__group1.screen3_animation'), $(window).scrollTop(), 'screen3_animation');
        }
        if($('.screen3__group2.screen3_animation').length) {
            checkCanMainPageScreensAnimation($('.screen3__group2.screen3_animation'), $(window).scrollTop(), 'screen3_animation');
        }
    });
});
$(window).on('load', function() {
    ...
    $(window).scroll(function() {
        if($('.title__text.screen4_animation').length) {
            checkCanMainPageScreensAnimation($('.title__text.screen4_animation'), $(window).scrollTop(), 'screen4_animation');
        }
        if($('.title__accentText.screen4_animation').length) {
            checkCanMainPageScreensAnimation($('.title__accentText.screen4_animation'), $(window).scrollTop(), 'screen4_animation');
        }
        if($('.screen4__text.screen4_animation').length) {
            checkCanMainPageScreensAnimation($('.screen4__text.screen4_animation'), $(window).scrollTop(), 'screen4_animation');
        }
        if($('.screen4__howWeWork.screen4_animation').length) {
            checkCanMainPageScreensAnimation($('.screen4__howWeWork.screen4_animation'), $(window).scrollTop(), 'screen4_animation');
        }
        if( $('.screen4_animation').length ) {
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        if($('.title__text.screen5_animation').length) {
            checkCanMainPageScreensAnimation($('.title__text.screen5_animation'), $(window).scrollTop(), 'screen5_animation');
        }
        if($('.title__accentText.screen5_animation').length) {
            checkCanMainPageScreensAnimation($('.title__accentText.screen5_animation'), $(window).scrollTop(), 'screen5_animation');
        }
        if($('.screen5__description.screen5_animation').length) {
            checkCanMainPageScreensAnimation($('.screen5__description.screen5_animation'), $(window).scrollTop(), 'screen5_animation');
        }
        if($('.screen5__btn.screen5_animation').length) {
            checkCanMainPageScreensAnimation($('.screen5__btn.screen5_animation'), $(window).scrollTop(), 'screen5_animation');
        }
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        ...
        if($('.clone1.screen6_animation').length) {
            checkCanMainPageScreensAnimation($('.clone1.screen6_animation'), $(window).scrollTop(), 'screen6_animation');
        }
        if($('.clone2.screen6_animation').length) {
            checkCanMainPageScreensAnimation($('.clone2.screen6_animation'), $(window).scrollTop(), 'screen6_animation');
        }
        if($('.screen6__description.screen6_animation').length) {
            checkCanMainPageScreensAnimation($('.screen6__description.screen6_animation'), $(window).scrollTop(), 'screen6_animation');
        }
        if($('.screen6__btn.screen6_animation').length) {
            checkCanMainPageScreensAnimation($('.screen6__btn.screen6_animation'), $(window).scrollTop(), 'screen6_animation');
        }
    });
...
$(document).ready(function() {
    ...
    $(window).scroll(function() {
        if($('.clone1.screen7_animation').length) {
            checkCanMainPageScreensAnimation($('.clone1.screen7_animation'), $(window).scrollTop(), 'screen7_animation');
        }
        if($('.clone2.screen7_animation').length) {
            checkCanMainPageScreensAnimation($('.clone2.screen7_animation'), $(window).scrollTop(), 'screen7_animation');
        }
        if($('.screen7__description.screen7_animation').length) {
            checkCanMainPageScreensAnimation($('.screen7__description.screen7_animation'), $(window).scrollTop(), 'screen7_animation');
        }
        if($('.screen7__group1.screen7_animation').length) {
            checkCanMainPageScreensAnimation($('.screen7__group1.screen7_animation'), $(window).scrollTop(), 'screen7_animation');
        }
        if($('.screen7__group2.screen7_animation').length) {
            checkCanMainPageScreensAnimation($('.screen7__group2.screen7_animation'), $(window).scrollTop(), 'screen7_animation');
        }
        if($('.screen7__group3.screen7_animation').length) {
            checkCanMainPageScreensAnimation($('.screen7__group3.screen7_animation'), $(window).scrollTop(), 'screen7_animation');
        }
    });
});
$(document).ready(function() {
    if($('.screen8_animation').length) {
        $(window).scroll(function () {
            if ($('.title__text.screen8_animation').length) {
                checkCanMainPageScreensAnimation($('.title__text.screen8_animation'), $(window).scrollTop(), 'screen8_animation');
            }
            if ($('.title__accentText.screen8_animation').length) {
                checkCanMainPageScreensAnimation($('.title__accentText.screen8_animation'), $(window).scrollTop(), 'screen8_animation');
            }
            if ($('.screen8__description.screen8_animation').length) {
                checkCanMainPageScreensAnimation($('.screen8__description.screen8_animation'), $(window).scrollTop(), 'screen8_animation');
            }
            if ($('.screen8__btn.screen8_animation').length) {
                checkCanMainPageScreensAnimation($('.screen8__btn.screen8_animation'), $(window).scrollTop(), 'screen8_animation');
            }
        });
    }
});
$(document).ready(function() {
    if($('.screenCommonBg').length) {
        ...
        var block = false;
        var kRoute = 0.3;
        var anim_route = $(".screen8__description").offset().top+ $(window).height()*0.2 - $(".screen7__group3").offset().top;
        var numFrames = 179;
        ...
        $(window).scroll(function() {
            ...
            if (scrollRoute < 0) {
                if ($(".screenCommonBg .title.clone1").offset().top <= ($(window).scrollTop() + $(window).height() * 1-$(window).height()/2)) {
                    $(".screenCommonBg__img").addClass("show");
                }
                if ($(".screen9__title").offset().top <= ($(window).scrollTop() + $(window).height() * 0.85)) {
                    $(".screenCommonBg__img").addClass("hide");
                    map.goToAndStop(numFrames, true);
                }
            }else{
                if ($(".screenCommonBg .title.clone1").offset().top >= ($(window).scrollTop() + $(window).height() * 1-100-$(window).height()/2)){
            ...
            if($(".screen8 .title").offset().top<=($(window).scrollTop()+$(window).height()*1.2)) {
                if (scrollRoute < 0) {
            ...
            if (scrollRoute < 0) {
                if ($(".screen8 .title").offset().top <= ($(window).scrollTop() + $(window).height() * 0.6)) {
    ...
    if($('.screen9_animation').length) {
        $(window).scroll(function () {
            if ($('.title__text.screen9_animation').length) {
                checkCanMainPageScreensAnimation($('.title__text.screen9_animation'), $(window).scrollTop(), 'screen9_animation');
            }
            if ($('.title__accentText.screen9_animation').length) {
                checkCanMainPageScreensAnimation($('.title__accentText.screen9_animation'), $(window).scrollTop(), 'screen9_animation');
            }
            if ($('.screen9__description.screen9_animation').length) {
                checkCanMainPageScreensAnimation($('.screen9__description.screen9_animation'), $(window).scrollTop(), 'screen9_animation');
            }
            if ($('.screen9__form.screen9_animation').length) {
                checkCanMainPageScreensAnimation($('.screen9__form.screen9_animation'), $(window).scrollTop(), 'screen9_animation');
            }
        });
    }
});
...
function initScriptToSelectField() { //ф-ия вызывается в компоненте productSelection
   ...
   function showSelectFieldList(elem) {
        var parent = elem.closest('.selectField'),
            icon = parent.find('.selectField__icon').length ? parent.find('.selectField__icon') : parent.find('.selectField__ico'),
   ...
   $(document).on('click', '.selectField__item', function() {
       ...
       if($(this).hasClass('type')) {
           ...
           if($(this).index() == 1 || $(this).index() == 2) {
               fieldLength.css({'pointer-events': 'none', 'opacity': .25});
           }else {
...
$(document).ready(function() {
    $(document).on('click', '.surface-types__card', function() {
        ...
        if ($(scroll_el).length != 0) { // проверим существование элемента чтобы избежать ошибки
            $('html, body').animate({ scrollTop: $(scroll_el).offset().top - headerHeight - 20 }, 500); // анимируем скроолинг к элементу scroll_el
        }
        return false; // выключаем стандартное действие
    });
});
    ...
    if( $('.tabs').length ) {
        $(document).on('click', '.tabs__title', function() {
            ...
            $('.tabs__item.active').fadeOut(250, function() {
                ...
                $('.tabs__item').eq(_$that.index()).fadeIn(250, function() {
                    $(this).addClass('active');
                });
            });
        });
    ...
    if( $('.vacancies__item').length ) {
            ...
            if( parent.hasClass('jQ_active') ) {
                ...
                if( $('.vacancies__item.jQ_active').length ) {
                    ...
                    $('.vacancies__item.jQ_active')
                        ...
                        .find('.vacancy__content').slideUp();

                    parent.addClass('jQ_active');

                    content.slideDown(function () {
                        $('html, body').animate({ scrollTop: title.offset().top - 100 }, 500); // анимируем скроолинг к элементу scroll_el
                    });
                }else {
                    parent.addClass('jQ_active');
        ...
        $(document)
            ...
            .on('click', '.vacancy__btn', function () {
                ...
                var text = $(this).closest('.vacancy').find('.vacancy__title').text().trim();

                $('#VACANCY_VACANCY').val(''+text).addClass('jQ_input_not_empty');
...
function removeDisclaimer() {
    if($.cookie('disable_disclaimer') && JSON.parse($.cookie('disable_disclaimer'))) {
            ...
            .on('keyup', function (event) {
               switch (event.keyCode) {
                   case 27: {
                       ...
                       $('.siteMap__switch').parents(".siteMap").find(".siteMap__popup").stop().fadeOut(500);
                   }
               }
            });
    }
